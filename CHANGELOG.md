...
- Fix self-update: replace bulk `Remove-Item -Recurse` + `Copy-Item -Recurse` with per-file copy, avoiding lock errors on Windows when updating `tools/` while the script is running.
- Add per-file live output during update (prefixed `upd`/`new`).
- Add per-file error trapping; errors accumulate and are listed in the update summary, which turns yellow when any copy fails.

## [2.5.4] — 2026-03-06

### Fixed

- Fix update dirty-file warning: exclude untracked (`??`) files from managed-path check so only tracked modified/deleted files are shown as needing attention before update.

## [2.5.3] — 2026-03-06

### Fixed

- Fix update mode: replace `git sparse-checkout set` with `git sparse-checkout disable` in `Get-UpdateSource` (PS) and `get_update_source` (bash) to correctly expand cone-mode sparse clones and update all managed paths, not just root-level files.

## [2.5.2] — 2026-03-06

### Changed

- Fix update flow menu re-render: anchor menu row before loop to prevent sub-content (changelog, file list) from pushing menu down on each iteration.

## [2.5.1] — 2026-03-06

### Changed

- Add direct-edit and temp-file-cleanup policies to all agents, spec.instructions, and copilot-instructions.
